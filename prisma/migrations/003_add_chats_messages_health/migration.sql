...
-- 4. Fix bot_logs foreign key to use SET NULL
-- (This may already be set, but ensure it's correct)
DO $$
BEGIN
  -- Check if constraint exists and update if needed
  IF EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE constraint_name = 'bot_logs_botId_fkey'
    AND table_name = 'bot_logs'
  ) THEN
    ALTER TABLE bot_logs DROP CONSTRAINT "bot_logs_botId_fkey";
    ALTER TABLE bot_logs ADD CONSTRAINT "bot_logs_botId_fkey"
      FOREIGN KEY ("botId") REFERENCES bots(id) ON DELETE SET NULL;
  END IF;
END $$;
